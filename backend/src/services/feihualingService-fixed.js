...
class FeihualingService {
  ...
  addUser(userId, username, socketId) {
    ...
    // 从数据库获取用户信息
    db.get(
      'SELECT class_id, (SELECT MAX(rounds) FROM feihua_battles WHERE player1_id = ? OR player2_id = ?) as max_rounds FROM users WHERE id = ?',
      [userId, userId, userId],
      (err, row) => {
        const classId = row ? (row.class_id || null) : null;
        const maxRounds = row ? (row.max_rounds || 0) : 0;
