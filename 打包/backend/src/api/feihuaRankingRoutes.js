...
router.get('/user/:userId', async (req, res) => {
  try {
    ...
    const info = await feihuaRankingService.getRankingInfo(parseInt(userId));
    const rankInfo = await feihuaRankingService.getUserRank(parseInt(userId));
    res.json({
      success: true,
      data: {
        ...info,
    ...
    res.status(500).json({ message: '获取用户段位信息失败' });
  }
});
