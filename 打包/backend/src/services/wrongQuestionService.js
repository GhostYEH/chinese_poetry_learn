...
若材料不足，可结合常识推断，但不要编造篇名作者。`;
  ...
  const wrongQuestionModel = 'Qwen/Qwen2.5-7B-Instruct';
  const wrongQuestionApiUrl = 'https://api.siliconflow.cn/v1/chat/completions';

  try {
    const response = await fetch(wrongQuestionApiUrl, {
      method: 'POST',
      headers: {
